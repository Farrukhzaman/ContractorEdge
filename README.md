...
1. Then `cd YOUR_PROJECT_REPO && cp .env.dist .env`
1. Next `npm ci && npm run dev`
...
1. Reinstall dependecies `npm ci --production`
1. Zip up the following folders and files
...
package.json
```
> Note: Installing packages on the server is unreliable and error prone so we have to include node_modules in our deployment.

Set the following Application Settings (Azure Portal)
```sh
# In addition to all the environment variables required by the app
PAYLOAD_CONFIG_PATH=dist/payload.config.js
NODE_ENV=production
WEBSITE_RUN_FROM_PACKAGE=1
```

Set the following "Startup Command"  (Azure Portal)
```
...
1. First, invoke the `payload build` script by running `npm run build` in your project root. This creates a `./build` directory with a production-ready admin bundle.
1. Then, run `npm run serve` to run Node in production and serve Payload from the `./build` directory.
